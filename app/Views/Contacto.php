<br>
<div class = "txt-pag">
<br>
    <h1 class = "titulos-contacto">Contacto</h1>
    <p><strong>¿Querés ponerte en contacto con nosotros?</strong></p>
    <p>En Xport estamos para ayudarte a encontrar el auto de tus sueños. 
       Si tenés consultas, querés agendar una visita o simplemente necesitás más información, podés comunicarte a través de los siguientes medios:</p>

    <div class="form-container">
    <form action="https://api.web3forms.com/submit" method="POST" class="contact-right">
        <div class="contact-right-title">
        <h2 class = "titulos-contacto">Contactanos</h2>
        <hr>
        </div>
        <input type="hidden" name="access_key" value="bf818953-ca3a-4d60-8224-8346c9b5770d">
        <input type="text" name="Nombre" placeholder="Tu nombre" class="contact-inputs" required>
        <input type="email" name="Correo" placeholder="Tu correo" class="contact-inputs" required>
        <textarea name="message" placeholder="Tu consulta" class="contact-inputs" required></textarea>
        <button type="submit">Enviar</button>
    </form>
    </div>

<br>
    <h2 class = "titulos-contacto">Medios de Contacto</h2>
    <ul style="list-style: none; padding: 0;">
        <li><strong>Teléfono:</strong> 3795323328</li>
        <li><strong>Email: </strong> XportContacto@gmail.com</li>
        <li><strong>RedesSociales:</strong>Puedes encontrarnos en todas las redes como Xport!</li>
    </ul>

<br>
    <h2 class = "titulos-contacto">Información Legal</h2>
    <ul style="list-style: none; padding: 0;">
        <li><strong>Razón Social:</strong> Xport</li>
        <li><strong>Titular: </strong>Marcos Santino Mazzanti y Joaquin Rojas Yacuzzi</li>
        <li><strong>Domicilio Legal:</strong> Yrigoyen 128</li>
    </ul>
    <p>También te invitamos a visitarnos personalmente en nuestro local,
       donde podrás ver todos los modelos disponibles y recibir atención personalizada de nuestro equipo especializado. ¡Te esperamos!</p>
</div>